    ...
    try:
        ...
        if current_secret_seq_num < update_call_seq_num:
            ...
            for crypt_item in disk_util.get_crypt_items():
                ...
                old_keyslot = crypt_item.current_luks_slot

                crypt_item.current_luks_slot = new_keyslot

                disk_util.update_crypt_item(crypt_item)
                updated_crypt_items.append((crypt_item, old_keyslot))
...
def clear_new_luks_keys(disk_util, old_passphrase, new_passphrase, bek_util, encryption_config, updated_crypt_items):
    try:
        ...
        executor = CommandExecutor(logger)
        for item in updated_crypt_items:
            crypt_item = item[0]
            if not crypt_item:
                continue
            ...
            crypt_item.current_luks_slot = item[1]
...
def handle_encryption(public_settings, encryption_status, disk_util, bek_util, encryption_operation):
    ...
    if extension_parameter.config_file_exists() and extension_parameter.config_changed():
        ...
        if not are_devices_encrypted:
            logger.log('Required devices not encrypted for volume type {0}. Calling update for already encrypted devices'.format(volume_type))
            update_encryption_settings(items_to_encrypt)
            logger.log('Settings updated for encrypted devices. Calling enable for new devices.')
            enable_encryption()
        else:
        ...
        if encryption_marker.config_file_exists():
            logger.log('Encryption marker exist. Calling Enable')
            enable_encryption()
        else:
...
def find_all_devices_to_encrypt(encryption_marker, disk_util, bek_util, volume_type=None, current_command=None):
    ...
    for device_item in device_items:
        ...
        if any(di.name == device_item.name for di in device_items_to_encrypt):
            continue
        if disk_util.should_skip_for_inplace_encryption(device_item, volume_type):
            continue
